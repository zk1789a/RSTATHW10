...
```{r}
US_by_state <- US %>% group_by(Province_State, Country_Region, date) %>% summarize(Confirmed_cases = sum(cases), Deaths = sum(Deaths), Lat = median(Lat), Long = median(Long), Population = sum(Population)) %>% mutate(Deaths_per_mill = (Deaths / Population) * 1000000) %>% select(Province_State, Country_Region, date, Confirmed_cases, Deaths, Deaths_per_mill, Population, Lat, Long) %>% ungroup() 
