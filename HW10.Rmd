...
deaths_global <- 
  deaths_global %>% 
  pivot_longer(cols = c(`1/22/20`:`10/27/20`), names_to = "date", values_to = "Deaths")
...
US_by_state <- US %>% group_by(Province_State, Country_Region, date) %>% summarize(Confirmed_cases = sum(cases_us$cases), Deaths = sum(Deaths), Lat = median(Lat), Long = median(Long), Population = sum(Population)) %>% mutate(Deaths_per_mill = Deaths * 1000000 / Population) %>% select(Province_State, Country_Region, date, Confirmed_cases, Deaths, Deaths_per_mill, Population, Lat, Long) %>% ungroup() 
```  
